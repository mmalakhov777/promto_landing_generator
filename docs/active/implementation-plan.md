...
- ✅ `scripts/generate_content.py` — AI-генерация через Anthropic API
- ✅ Промт-шаблон с keyword, нишевой аудиторией, инструкции по формату
- ✅ Генерируемые поля: SEO (meta_title <60, meta_description <160, h1, og_title, og_description), Hero, advantages (icon + title + description), how_it_works, reviews (4-5), faq (5-6), CTA-тексты
- ✅ Дефолтные данные из SiteSettings: social_proof, pricing (заполняются если null в ответе)
- ✅ Примеры работ: Unsplash image pool по категориям (restaurant, cafe, shop, portfolio, clinic, beauty + default)
- ✅ Rate limiting: пауза 1.5 сек между запросами, retry 3 попытки, exponential backoff
- ✅ Progress tracking: `generation_progress.json`, флаг `--resume`
- ✅ Dry-run режим: `--dry-run` — вывод без сохранения
- ✅ Использует Claude Sonnet (`claude-sonnet-4-20250514`)
...
- ⚠️ Тесты — отложены (инфраструктура тестирования не настроена)
